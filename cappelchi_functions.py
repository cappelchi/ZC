...
from backtesting import Strategy as vStrategy
from backtesting import Backtest as vBacktest
from backtesting import Orders as vOrders
from backtesting import Position as vPosition
...
'''
!test -d ZC || git clone https://github.com/cappelchi/ZC.git
if not 'ZC' in sys.path:
  sys.path += ['ZC']
!ls ZC
'''

'''
df_zc_60min_ba = pd.read_csv('ZC/ZC_60min_OHLC_time_roll_time_ba.csv',                       
                       #index_col = ('Date'),
                       parse_dates = True, 
                      )
print (df_zc_60min_ba.head())
print(np.shape(df_zc_60min_ba))
print(df_zc_60min_ba[(df_zc_60min_ba.Open == 0) | (df_zc_60min_ba.High == 0) | (df_zc_60min_ba.Low == 0) | (df_zc_60min_ba.Close == 0)])
df_zc_60min_ba.Low = np.where(df_zc_60min_ba.Low == 0, df_zc_60min_ba.Close, df_zc_60min_ba.Low)
df_zc_60min_ba.Open = np.where(df_zc_60min_ba.Open == 0, df_zc_60min_ba.Close, df_zc_60min_ba.Open)
print(df_zc_60min_ba[(df_zc_60min_ba.Open == 0) | (df_zc_60min_ba.High == 0) | (df_zc_60min_ba.Low == 0) | (df_zc_60min_ba.Close == 0)])
'''

'''
TakeUp = 1.149 #@param {type:"number"}
TrailDown = 1.047 #@param {type:"number"}
Labeling = True #@param ["True", "False"] {type:"raw"}
Bars, Prft, Trds, Y, trace = long_label_equity(df_zc_60min_ba, TakeUp, TrailDown, setY = Labeling)
print()
print('Результаты разметки')
print (f'TakeUp = {TakeUp}, TrailDown = {TrailDown}')
print(f'Кол-во бар просмотрено = {Bars}, Прибыль на 1$ входа = {Prft}')
print (f'Потенциальных сделок = {Trds}, Размечено бар = {Y.sum()}')                            
df = pd.Series()
df_zc_60min_ba['Y'] = pd.Series(Y.values, index = df_zc_60min_ba.index) 
'''

#def OptStrategy (Signal):
#    return pd.Series(Signal).shift(-1)

'''
class optimStrategy(vStrategy):    
    ...
    def init(self):        
        # Precompute value
        self.vStrat = self.I(OptStrategy, self.data.Y)       
        ...
        if not self.position:
            ...
            if self.position.is_long:
                ...
                if self.vStrat == -1:
                    self.position.close()
                    if self.printquot:
                        self.prt('Cover Short')
     
    def prt(self, msg):
        
        print (msg, ' by {}'.format(self.data.Close[-1]), end = ' ')                    
        if self.printdate:
            print (' at {}'.format(self.data.index[-1]), end = ' ')
            if self.printquot:
                print (' with thr: {}'.format(self.vStrat[-1]), end = ' ')
        print ('')    
         '''

#optbt = vBacktest(df_zc_60min_ba, optimStrategy, cash=20000, commission=.005)
#stats = optbt.run()
#print(stats)

#output_notebook()
#optbt.plot(plot_width=1200, plot_drawdown=True)

#trace[(trace.index > 5290) & (trace.index < 5350)]
